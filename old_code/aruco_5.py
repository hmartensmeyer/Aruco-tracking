    ...
    UNWRAP_PLOT_ANGLES = True        # Whether to unwrap angles in the plot to avoid +/-180 degree jumps
    ...
    KF_MEASUREMENT_NOISE_POS = 0.02
    KF_MEASUREMENT_NOISE_ORI = 0.05
    ...
    class RealTimePlotter:
        ...
        Includes logic to unwrap angle data to prevent +/-180 degree jumps.
        """
        def __init__(self, history_len, plot_width_px, video_height_px, buoy_ids_to_plot, is_relative):
            ...
            self.buoy_data = {}
            self.buoy_lines = {}
            # This new dictionary will store the state needed for unwrapping
            self.last_angles = {}
            colors = plt.cm.jet(np.linspace(0, 1, max(1, len(buoy_ids_to_plot))))
            ...
            rot_title = "Buoy CoG Orientation (Unwrapped)" if UNWRAP_PLOT_ANGLES else "Buoy CoG Orientation (Euler XYZ)"
            ...
            for i, buoy_id in enumerate(buoy_ids_to_plot):
                ...
                rot_lines = {
                    'x': self.ax_rot.plot([], [], label=f'{label} Roll', color=color, linestyle='-', linewidth=PLOT_LINE_WIDTH)[0],
                    'y': self.ax_rot.plot([], [], label=f'{label} Pitch', color=color, linestyle='--', linewidth=PLOT_LINE_WIDTH)[0],
        ...
        def _unwrap_angle(self, current_raw, last_raw, last_unwrapped):
            """Unwraps a single angle value."""
        ...
        def update_and_get_plot_image(self, new_data_dict, timestamp):
            ...
            for buoy_id, data in new_data_dict.items():
                ...
                # --- Rotation Data (with unwrapping logic) ---
                raw_rot = data['rot']
                unwrapped_rot = list(raw_rot) # Start with raw values

                if UNWRAP_PLOT_ANGLES:
                    if buoy_id not in self.last_angles:
                        # First data point for this buoy, initialize state
                        self.last_angles[buoy_id] = {
                            'raw': {'x': raw_rot[0], 'y': raw_rot[1], 'z': raw_rot[2]},
                            'unwrapped': {'x': raw_rot[0], 'y': raw_rot[1], 'z': raw_rot[2]}
                        }
                    else:
                        # We have history, so we can unwrap
                        last = self.last_angles[buoy_id]
                        unwrapped_rot[0] = self._unwrap_angle(raw_rot[0], last['raw']['x'], last['unwrapped']['x'])
                        unwrapped_rot[1] = self._unwrap_angle(raw_rot[1], last['raw']['y'], last['unwrapped']['y'])
                        unwrapped_rot[2] = self._unwrap_angle(raw_rot[2], last['raw']['z'], last['unwrapped']['z'])
                        
                        # Update the state for the next frame
                        self.last_angles[buoy_id]['raw'] = {'x': raw_rot[0], 'y': raw_rot[1], 'z': raw_rot[2]}
                        self.last_angles[buoy_id]['unwrapped'] = {'x': unwrapped_rot[0], 'y': unwrapped_rot[1], 'z': unwrapped_rot[2]}
                
                # Append the (potentially unwrapped) rotation data to the plot buffer
                self.buoy_data[buoy_id]['rot_x'].append(unwrapped_rot[0])
                self.buoy_data[buoy_id]['rot_y'].append(unwrapped_rot[1])
                self.buoy_data[buoy_id]['rot_z'].append(unwrapped_rot[2])
    ...
    def _process_frame_common(
        ...
        if ids is not None and len(ids) > 0:
            ...
            for buoy_id, estimates_for_this_buoy in detected_markers_for_buoy_fusion.items():
                ...
                # --- Store data for CSV and Plotter ---
                if current_log_tvec_cog is not None and euler_angles_cog_for_log_deg is not None:
                    plot_data_for_frame[buoy_id] = {
                        'pos': current_log_tvec_cog.flatten(),
                        'rot': euler_angles_cog_for_log_deg.flatten()
                    }
                ...
                try:
                    ...
                    # Draw larger axes and text for 4K
                    ...
                    y_off = -80 # Increased vertical offset for larger fonts
                    text_y_step = int(35 * OVERLAY_FONT_SCALE) # Scaled spacing
                    text_x_offset = 20
