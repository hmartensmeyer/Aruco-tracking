...
- Integrated GUI: A modern, themed Tkinter-based graphical user interface provides
  easy access to all functionalities, including live/offline processing, calibration
  tools, and settings configuration.
- Calibration Tools:
...
- sv-ttk (recommended for modern theme): pip install sv-ttk

Author: Paul FROMONT
Date: 17/06/2025
Version: 0.9 (Professional GUI Refresh)
...
# %% ===========================================================================
# CONFIGURATION PARAMETERS (No changes from original)
# =============================================================================
...
APP_ICON_PATH = 'aruco_icon.png'  # Path for the application icon
...
OFFLINE_PREVIEW_RESIZE_FACTOR = 1  # Resize factor for the preview window (e.g., 0.25 for 4K -> 1080p).

# ... [All other non-GUI functions from the original script are unchanged and go here] ...
# %% ===========================================================================
...
# %% ===========================================================================
# UTILITY AND CORE MATHEMATICAL FUNCTIONS (No changes from original)
# =============================================================================
...
def rotation_vector_to_euler(rvec):
    ...
    Returns:
        tuple: A tuple containing:
            - np.ndarray: 3-element vector of Euler angles in degrees (X, Y, Z).
...
def get_stable_orientation_angles_from_matrix(rot_matrix):
    ...
    # We use arcsin(-z) because if body_x_in_world[2] is positive, the body is pointing down (negative pitch).
...
# %% ===========================================================================
# CORE FRAME PROCESSING LOGIC (No changes from original)
# =============================================================================
...
def _process_frame_common(
    ...
    """
    Processes a single video frame to detect and track buoys (COMMON LOGIC).
    This function is the core of the tracking logic, refactored to be called
    by both live and offline processing modes.
    ... [Function body is identical to original] ...
    """
    cog_log_entries = []
    ...
    if ids is not None and len(ids) > 0:
        ...
        rvecs_all, tvecs_all, _ = aruco.estimatePoseSingleMarkers(
            refined_corners_list, marker_size_m, camera_matrix, dist_coeffs
        )

        # --- Find Reference Marker (if enabled) ---
        if use_reference_marker_flag:
            try:
                ...
                pass # Reference marker not in this frame's detections

        # --- Process each detected marker for individual logging and buoy association ---
        for i, marker_id in enumerate(ids.flatten()):
            r_cam_marker, t_cam_marker = rvecs_all[i].reshape(3,1), tvecs_all[i].reshape(3,1)

            # Store absolute pose data for this marker
            ...
            # If reference is used and found, calculate and store relative pose
            if use_reference_marker_flag and ref_marker_detected_this_frame:
                ...
                except Exception: pass # Handle potential transform errors
            
            all_markers_data_this_frame[marker_id] = marker_data

        # --- Log Individual Marker Data (if enabled) ---
        if log_all_marker_data_flag:
            for marker_id, data in all_markers_data_this_frame.items():
                ...
                log_row.extend([f"{v:.6f}" for v in data['cam_quat']])
                ...
                if use_reference_marker_flag:
                    if 'rel_tvec' in data:
                        ...
                        log_row.extend([f"{v:.6f}" for v in data['rel_quat']])
                    else:
                        log_row.extend(["NaN"] * (3 + 3 + 4))
                individual_marker_log_entries.append(log_row)

        # --- Group detected markers by buoy and estimate buoy pose ---
        buoy_pose_estimates = {}
        marker_obj_pts = np.array([[-marker_size_m/2, marker_size_m/2, 0], [marker_size_m/2, marker_size_m/2, 0],
                                   [marker_size_m/2, -marker_size_m/2, 0], [-marker_size_m/2, -marker_size_m/2, 0]], dtype=np.float32)
        ...
        for marker_id, data in all_markers_data_this_frame.items():
            ...
            # Estimate buoy frame pose (BRF) from this one marker (Xi)
            R_Xi_BRF = R_BRF_Xi.T
            ...
            buoy_pose_estimates.setdefault(buoy_id, []).append({
                'R_est': R_cam_BRF_est, 't_est': t_cam_BRF_est, 'confidence': confidence
            })
            
        # --- Fuse poses for each buoy and calculate final CoG pose ---
        for buoy_id, estimates in buoy_pose_estimates.items():
            ...
            # Weighted fusion of translations and rotations
            ...
            # --- Calculate CoG pose in camera frame ---
            ...
            # --- Prepare data for logging and plotting ---
            ...
            if use_reference_marker_flag and ref_marker_detected_this_frame:
                try:
                    ...
                    # R_ref_cog = R_ref_cam^T * R_cam_cog
                    final_R_for_log = R_ref_cam.T @ R_cog_cam
                    # t_ref_cog = R_ref_cam^T * (t_cog_cam - t_ref_cam)
            ...
            if use_reference_marker_flag:
                ...
                if is_relative_log:
                    ...
                    log_entry.extend(["NaN"] * (3 + 3 + 4))
            # Always log absolute camera pose as well
            ...
            try:
                ...
                y_off, y_step = -80, int(35 * OVERLAY_FONT_SCALE)
    ...
    return (annotated_frame, plot_image, cog_log_entries, individual_marker_log_entries, 
            positions_history, detected_buoys_summary, ref_marker_detected_this_frame)


# %% ===========================================================================
# TOP-LEVEL APPLICATION MODES (No changes from original)
# =============================================================================
...
def track_buoy_with_aruco_6dof(parent_tk_window=None):
    ...
    This function acts as a 'wrapper' around the core processing logic.
    ... [Function body is identical to original] ...
    """
    # --- Setup: Directories, Camera, Log Files, Video Writer ---
    ...
    try:
        while True:
            ...
            # --- Live-Specific Tasks: Logging, Display, Input Handling ---
            ...
            info_panel_drawer.draw(annotated_frame, proc_fps, (w, h), cam_fps, (is_recording, recording_path),
                                   (USE_REFERENCE_MARKER, ref_detected, REFERENCE_MARKER_ID), buoy_summary, is_live=True)
            ...
            cv2.putText(final_frame, f"Q: Quit | R: Reverse Disp | V: Record", (20, h-40), cv2.FONT_HERSHEY_SIMPLEX, OVERLAY_FONT_SCALE, (0,165,255), OVERLAY_THICKNESS)
...
def process_video_offline(input_video_path, use_ref_marker_override,
    ...
    """
    Processes a pre-recorded video file. This function acts as a 'wrapper'
    around the core processing logic.
    ... [Function body is identical to original] ...
    """
    ...
    # --- Setup: Directories, Video Reader/Writer, Log Files ---
    experiment_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    if video_start_date_str and video_start_time_str:
        try: 
            ...
            messagebox.showwarning("Date/Time Parse Error", "Could not parse date/time. Using relative time.", parent=parent_tk)
    ...
    try:
        while True:
            ...
            # --- Offline-Specific Tasks: Logging, Video Writing, GUI Update ---
            if cog_logs: csv.writer(cog_csv_file).writerows(cog_logs)
            if marker_logs and markers_csv_file: csv.writer(markers_csv_file).writerows(marker_logs)
            
            proc_fps = frame_count / (time.time() - start_proc_time) if time.time() > start_proc_time else 0.0
            info_panel_drawer.draw(annotated_frame, proc_fps, (w, h), fps, (vid_writer is not None, annotated_vid_path),
                                   (use_ref_marker_override, ref_detected, REFERENCE_MARKER_ID), buoy_summary, is_live=False)
...
# %% ===========================================================================
# NEW/IMPROVED GUI CLASSES
# =============================================================================

# --- NEW: A helper class for creating tooltips ---
class ToolTip:
...
# --- NEW: A professional-looking custom dialog for user input ---
class CustomInputDialog(tk.Toplevel):
    """A custom, professional-looking dialog for various inputs."""
        ...
        for field in self.fields:
            ...
            try:
                ...
                if "min" in validation_rules and val < validation_rules["min"]:
                    messagebox.showwarning("Validation Error", f"{label_text} must be at least {validation_rules['min']}.", parent=self)
                    return False
                if "max" in validation_rules and val > validation_rules["max"]:
                    messagebox.showwarning("Validation Error", f"{label_text} must be no more than {validation_rules['max']}.", parent=self)
                    return False
            except tk.TclError:
                 messagebox.showwarning("Validation Error", f"Invalid value for {label_text}.", parent=self)
                 return False
        return True
...
class OfflineProcessingGUI(tk.Toplevel):
    ...
    def __init__(self, master):
        ...
        ToolTip(self.browse_button, "Browse for a video file to process.")
        ...
        self.ref_marker_check = ttk.Checkbutton(options_frame, text=f"Use Reference Marker (ID: {REFERENCE_MARKER_ID}) for this run", variable=self.ref_marker_var)
        self.ref_marker_check.pack(pady=5, anchor='w')
        ToolTip(self.ref_marker_check, "If checked, all buoy poses will be calculated relative to the reference marker.")

        time_input_frame = ttk.LabelFrame(self, text="Optional: Video Start Timestamp", padding="10")
        ...
        ToolTip(time_input_frame, "If provided, timestamps in the log file will be absolute, starting from this date and time.")
        ...
        ToolTip(self.process_button, "Begin processing the selected video file.")
...
class SettingsDialog(tk.Toplevel):
    ...
    def __init__(self, master):
        super().__init__(master)
        self.title("Application Settings"); self.transient(master); self.grab_set()
        ...
        c1 = ttk.Checkbutton(vis_frame, variable=self.show_plots_var, text="Enable side panel with real-time plots")
        c1.pack(anchor='w', pady=2); ToolTip(c1, "Show/hide the live data plots during tracking sessions.")
        c2 = ttk.Checkbutton(vis_frame, variable=self.reverse_display_var, text="Reverse live camera display by default")
        c2.pack(anchor='w', pady=2); ToolTip(c2, "Flips the live camera view horizontally. Useful for 'selfie' camera orientations.")

        # Reference, Recording & Logging Frame
        log_frame = ttk.LabelFrame(main_frame, text="Tracking & Logging", padding="10")
        log_frame.pack(fill=tk.X, pady=5)
        c3 = ttk.Checkbutton(log_frame, variable=self.use_ref_marker_var, text="Enable Reference Marker by default", command=self._toggle_ref_id_entry)
        c3.pack(anchor='w', pady=2); ToolTip(c3, "Enable/disable reporting poses relative to a static world-frame marker.")
        
        ref_id_frame = ttk.Frame(log_frame)
        ref_id_frame.pack(fill=tk.X, padx=20)
        ttk.Label(ref_id_frame, text="Reference Marker ID:").pack(side=tk.LEFT)
        self.ref_id_entry = ttk.Entry(ref_id_frame, textvariable=self.ref_marker_id_var, width=7); self.ref_id_entry.pack(side=tk.LEFT)
        ToolTip(self.ref_id_entry, "The ArUco ID of the marker to be used as the static reference frame.")

        c4 = ttk.Checkbutton(log_frame, variable=self.record_video_var, text="Record video by default (Live mode)")
        c4.pack(anchor='w', pady=2); ToolTip(c4, "If checked, live tracking sessions will start recording automatically.")
        c5 = ttk.Checkbutton(log_frame, variable=self.log_all_marker_data_var, text="Log all individual marker data")
        c5.pack(anchor='w', pady=2); ToolTip(c5, "Creates an extra, detailed CSV log for every single marker detected in each frame.")
        ...
        ttk.Label(g, text="Camera Index:").grid(row=0, column=0, sticky='w', pady=2); e1 = ttk.Entry(g, textvariable=self.camera_index_var, width=10); e1.grid(row=0, column=1, sticky='w')
        ToolTip(e1, "The system index of the camera (e.g., 0, 1, 2...).")
        ttk.Label(g, text="ArUco Marker Size (m):").grid(row=1, column=0, sticky='w', pady=2); e2 = ttk.Entry(g, textvariable=self.marker_size_var, width=10); e2.grid(row=1, column=1, sticky='w')
        ToolTip(e2, "The physical side length of the square ArUco markers, in meters.")
        ttk.Label(g, text="Camera Resolution:").grid(row=2, column=0, sticky='w', pady=2)
        res_frame = ttk.Frame(g); res_frame.grid(row=2, column=1, sticky='w')
        e3 = ttk.Entry(res_frame, textvariable=self.cam_res_width_var, width=7); e3.pack(side=tk.LEFT)
        ttk.Label(res_frame, text=" x ").pack(side=tk.LEFT)
        e4 = ttk.Entry(res_frame, textvariable=self.cam_res_height_var, width=7); e4.pack(side=tk.LEFT)
        ToolTip(res_frame, "The desired camera capture resolution (width x height).")

        # Buttons
        btn_fr = ttk.Frame(main_frame); btn_fr.pack(pady=(15,5))
        ttk.Button(btn_fr, text="Apply & Save", command=self.apply_settings, style="Accent.TButton").pack(side=tk.LEFT, padx=5)
    ...
    def apply_settings(self):
        ...
        if n_use_ref:
            try:
                ...
                else: messagebox.showwarning("Invalid ID","Reference marker ID must be a non-negative integer.",parent=self); self.ref_marker_id_var.set(self.initial_ref_marker_id); return
            except tk.TclError: messagebox.showwarning("Invalid ID","Reference marker ID must be an integer.",parent=self); self.ref_marker_id_var.set(self.initial_ref_marker_id); return

        try:
            n_cam_idx = self.camera_index_var.get(); n_marker_size = self.marker_size_var.get()
            n_cam_w = self.cam_res_width_var.get(); n_cam_h = self.cam_res_height_var.get()
            if n_cam_idx < 0: raise ValueError("Camera index must be >= 0.")
            if n_marker_size <= 0: raise ValueError("Marker size must be > 0.")
            if n_cam_w <= 0 or n_cam_h <= 0: raise ValueError("Camera resolution must be positive.")
        except (tk.TclError, ValueError) as e:
             messagebox.showwarning("Invalid Input", f"Error in hardware parameters: {e}", parent=self); return

        SHOW_REALTIME_PLOTS = self.show_plots_var.get()
        USE_REFERENCE_MARKER, REFERENCE_MARKER_ID = n_use_ref, v_ref_id
        RECORD_VIDEO, REVERSE_CAMERA_DISPLAY = self.record_video_var.get(), self.reverse_display_var.get()
        ...
        initialize_buoy_configs()
        messagebox.showinfo("Settings Applied","Global settings have been updated and will be used for subsequent sessions.",parent=self)
        self.destroy()


# --- REWRITTEN: The main application GUI window with a modern layout ---
class BuoyTrackerApp:
    ...
    def __init__(self, master):
        ...
        # --- Apply Theme ---
        if THEME_AVAILABLE:
            sv_ttk.set_theme("light") # or "light"
        ...
        # --- Header ---
        header_label = ttk.Label(main_frame, text="Buoy Tracking & Analysis Tool", font=("", 16, "bold"))
        ...
        live_btn = ttk.Button(core_frame, text="Start Live Tracking Session", command=self.ui_live_tracking, style="Accent.TButton")
        live_btn.pack(fill=tk.X, pady=5, ipady=8)
        ToolTip(live_btn, "Begin a real-time tracking session using a connected camera.")
        
        offline_btn = ttk.Button(core_frame, text="Process Recorded Video", command=self.ui_launch_offline_processor)
        offline_btn.pack(fill=tk.X, pady=5, ipady=4)
        ToolTip(offline_btn, "Analyze a pre-recorded video file to extract buoy poses.")

        # --- Calibration Tools ---
        calib_frame = ttk.LabelFrame(main_frame, text="Calibration Tools", padding="10")
        ...
        ToolTip(cam_calib_btn, "Calibrate camera intrinsics using a chessboard pattern.")
        ...
        ToolTip(buoy_calib_btn, "Calibrate the relative positions of markers on a buoy assembly.")

        # --- Status Bar ---
        self.status_var = tk.StringVar(value="Ready. Welcome to the ArUco Buoy Tracker.")
        status_bar = ttk.Frame(master, style="Card.TFrame", padding=5)
    ...
    def _create_menubar(self):
        ...
        gen_menu.add_command(label="Single ArUco Marker...", command=self.ui_generate_marker)
        gen_menu.add_command(label="Reference Marker...", command=self.ui_generate_reference_marker)
        gen_menu.add_command(label="Chessboard...", command=self.ui_generate_chessboard)
    ...
    def ui_show_about(self):
        about_text = (
            "ArUco-Based 6-DOF Buoy Pose Estimation and Tracking Tool\n"
            f"Version: 1.0 (GUI Refresh)\n"
            f"Author: {__doc__.split('Author: ')[1].splitlines()[0]}\n"
            f"Contact: {__doc__.split('Contact: ')[1].splitlines()[0]}\n\n"
            "This toolkit provides a comprehensive suite for high-precision\n"
            "6-DOF pose estimation of rigid bodies using ArUco markers."
        )
        messagebox.showinfo("About Buoy Tracker", about_text, parent=self.master)
    
    def ui_generate_marker(self):
        dict_size = 250 # Based on ARUCO_DICT_TYPE
        fields = [
            {"label": "Marker ID", "type": "int", "initial": 0, "validation": {"min": 0, "max": dict_size - 1}},
            {"label": "Image Size (px)", "type": "int", "initial": 600, "validation": {"min": 50, "max": 4000}},
            {"label": "Border Width (px)", "type": "int", "initial": 50, "validation": {"min": 0, "max": 500}}
        ]
        dialog = CustomInputDialog(self.master, "Generate ArUco Marker", fields)
        if dialog.result:
            mid, spx, bpx = dialog.result.values()
            self.status_var.set(f"Generating marker ID {mid}..."); self.master.update_idletasks()
    ...
    def ui_generate_reference_marker(self):
        ref_id = REFERENCE_MARKER_ID
        fields = [
            {"label": "Image Size (px)", "type": "int", "initial": 600, "validation": {"min": 50, "max": 4000}},
            {"label": "Border Width (px)", "type": "int", "initial": 50, "validation": {"min": 0, "max": 500}}
        ]
        dialog = CustomInputDialog(self.master, f"Generate Reference Marker (ID: {ref_id})", fields)
        if dialog.result:
            spx, bpx = dialog.result.values()
            self.status_var.set(f"Generating reference marker ID {ref_id}..."); self.master.update_idletasks()
            img = generate_aruco_marker(ref_id, spx, bpx, True, self.master)
            if img is not None:
                cv2.imshow(f"Generated Reference Marker - ID:{ref_id}", img)
                cv2.waitKey(0); cv2.destroyWindow(f"Generated Reference Marker - ID:{ref_id}")
            self.status_var.set("Ready.")

    def ui_generate_chessboard(self):
        sx, sy = CALIBRATION_CHESSBOARD_SHAPE
        fields = [{"label": "Square Size (px)", "type": "int", "initial": 100, "validation": {"min": 20, "max": 500}}]
        dialog = CustomInputDialog(self.master, f"Generate {sx}x{sy} Chessboard", fields)
        if dialog.result:
            spx = list(dialog.result.values())[0]
            self.status_var.set("Generating chessboard..."); self.master.update_idletasks()
    ...
    def ui_calibrate_camera(self):
        ...
        messagebox.showinfo("Camera Calibration",
            "The camera view will now open.\n\n"
            "- Point the camera at the chessboard pattern.\n"
            "- Press 'c' to capture a good view.\n"
            "- Press 'r' to reverse the display.\n"
            "- Press 'q' to finish and perform calibration.\n\n"
            "Follow the prompts in the console/terminal for more info.",
            parent=self.master)
        calibrate_camera(self.master)
        self.status_var.set("Ready.")

    def ui_live_tracking(self):
        self.status_var.set("Preparing live tracking session..."); self.master.update_idletasks()
        log_all_text = "Enabled (separate detailed marker CSV)" if LOG_ALL_MARKER_DATA else "Disabled (CoG data only)"
        conf_msg = (
            "You are about to start a live tracking session with the following settings:\n\n"
            f"  • Camera Index: {CAMERA_INDEX} @ {CAMERA_RESOLUTION[0]}x{CAMERA_RESOLUTION[1]}\n"
        ...
        if messagebox.askyesno("Confirm Live Session", conf_msg, parent=self.master):
            ...
            try:
                track_buoy_with_aruco_6dof(self.master)
            except Exception as e:
                messagebox.showerror("Live Tracking Error", f"An unexpected error occurred: {e}", parent=self.master)
                self.status_var.set(f"Live tracking error: {e}")
            self.status_var.set("Ready.")
        else:
            self.status_var.set("Live tracking cancelled.")

    def ui_launch_offline_processor(self):
        self.status_var.set("Opening Offline Video Processor..."); self.master.update_idletasks()
        opg = OfflineProcessingGUI(self.master)
        opg.protocol("WM_DELETE_WINDOW", lambda: [self.status_var.set("Ready."), opg.destroy()])
    ...
    def ui_calibrate_buoy_geometry(self):
        ...
        if not buoy_ids_available:
            messagebox.showerror("Error", "No buoys are defined in the configuration (BUOY_ID_TO_MARKER_IDS_MAP).", parent=self.master)
            return

        num_other_markers = len(BUOY_ID_TO_MARKER_IDS_MAP.get(buoy_ids_available[0], [0,0])) - 1
        num_frames_suggestion = BUOY_GEOMETRY_CALIB_MIN_SAMPLES_PER_PAIR * max(1, num_other_markers) * 2
        
        fields = [
            {"label": "Buoy ID to Calibrate", "type": "int", "initial": buoy_ids_available[0], "options": buoy_ids_available},
            {"label": "Total Views to Capture", "type": "int", "initial": max(10, num_frames_suggestion), "validation": {"min": 5, "max": 500}}
        ]
        ...
        if dialog.result:
            ...
            self.status_var.set(f"Buoy {buoy_id_to_calibrate} geometry calibration {'finished successfully.' if success else 'failed or was aborted.'}")
