...
# Camera Setup
CAMERA_INDEX = 0
#CAMERA_RESOLUTION = (3840, 2160) # Adjusted for typical webcam, change as needed
CAMERA_RESOLUTION = (1920, 1080) # Adjusted for typical webcam, change as needed
CAMERA_FPS = 30
REVERSE_CAMERA_DISPLAY = True

# Video Recording Settings
RECORD_VIDEO = False # For live mode, record annotated video
VIDEO_CODEC = 'mp4v'
VIDEO_FPS = 30
VIDEO_EXTENSION = '.mp4'
...
MARKER_SIZE_METERS = 0.1577 # Example: 3.3 cm markers
...
# Buoy Configuration
NUM_BUOYS = 9 # Reduced for easier testing, can be increased
MARKERS_PER_BUOY = 3
...
# Triangular Marker Fusion Configuration (Idealized Geometry)
USE_TRIANGULAR_MARKER_FUSION = True # This flag's role is diminished if calibrated geometry is used
TRIANGLE_SIDE_LENGTH_METERS = 0.21 # Example: 5cm side length for ideal triangle

# Buoy Center of Gravity (CoG) Offset Configuration
BUOY_COG_OFFSET_METERS = 0.23168 # Example: 5cm
...
MARKER_RELATIVE_GEOMETRY = { # Key is role index (0 for X0, 1 for X1, 2 for X2)
...
CHESSBOARD_SQUARE_SIZE_MM = 26.9 # Example: 20mm
BUOY_GEOMETRY_CALIB_MIN_SAMPLES_PER_PAIR = 5 # Min samples for each X0-Xi pair during buoy geometry calibration
...
OFFLINE_PREVIEW_RESIZE_FACTOR = 0.35
...
# Logging Option
LOG_ALL_MARKER_DATA = False # If True, creates a separate detailed CSV for individual markers
...
def rotation_vector_to_euler(rvec):
    """Converts a rotation vector to Euler angles (degrees) and Quaternion [x, y, z, w]."""
    if rvec is None:
        return np.array([[0.],[0.],[0.]]), R.identity().as_quat() 
    rvec_np = np.asarray(rvec).reshape(3) 
    if np.allclose(rvec_np, 0): # Check if rotation vector is effectively zero
        return np.array([[0.],[0.],[0.]]), R.identity().as_quat()
    try:
        rotation = R.from_rotvec(rvec_np)
        # Euler angles in XYZ intrinsic order, in radians, then converted to degrees
        ...
        # print(f"Error in rotation_vector_to_euler for rvec {rvec_np}: {e}") # Optional: for debugging
        return np.array([[0.],[0.],[0.]]), R.identity().as_quat()
...
def transform_pose_to_reference(rvec_obj_cam, tvec_obj_cam, rvec_ref_cam, tvec_ref_cam):
    ...
    # Transformation from Camera to Reference (inverse of Reference to Camera)
    # T_ref_cam = inv(T_cam_ref)
    # R_cam_ref = R_ref_cam
    # t_cam_ref = tvec_ref_cam
    R_ref_cam_T = R_ref_cam.T # R_cam_ref_inv = R_ref_cam_T
    t_cam_ref_inv_in_ref_coords = -R_ref_cam_T @ tvec_ref_cam_np # t_cam_ref_inv = -R_cam_ref_inv * t_cam_ref
    
    T_ref_cam = np.eye(4)
    T_ref_cam[:3,:3] = R_ref_cam_T
    T_ref_cam[:3,3] = t_cam_ref_inv_in_ref_coords.flatten()

    # Transformation from Object to Reference
...
def average_quaternions_weighted(quaternions, weights):
    """ Averages quaternions using a weighted method. """
    if not quaternions: return R.identity().as_quat()
    if len(quaternions) == 1: return quaternions[0]

    # Ensure quaternions are normalized and weights are positive
    normalized_quaternions = []
    valid_weights = []
    for q, w_val in zip(quaternions, weights):
        if w_val > 1e-9: # Only consider if weight is significant
            norm_q = np.linalg.norm(q)
            if norm_q < 1e-9: # Avoid division by zero for zero quaternion
                continue 
            q_norm = q / norm_q
    ...
    # Align quaternions to the first one to prevent averaging issues with antipodal quaternions
    ...
    for i in range(1, len(normalized_quaternions)):
        if np.dot(q_ref, normalized_quaternions[i]) < 0:
            normalized_quaternions[i] *= -1 # Flip to the "closer" hemisphere

    # Weighted average of quaternions (eigenvector method - more robust for larger spreads)
    # Construct the matrix M
    M = np.zeros((4, 4))
    for q_i, w_i in zip(normalized_quaternions, valid_weights):
        q_i = q_i.reshape(4,1) # Ensure q_i is a column vector
        M += w_i * (q_i @ q_i.T) # Outer product

    # The average quaternion is the eigenvector corresponding to the largest eigenvalue of M
...
def _process_frame_common(
    ...
    positions_history, rotations_history, # trajectory_length_config is now global TRAJECTORY_LENGTH
    ...
    if ids is not None and len(ids) > 0:
        ...
        if log_all_marker_data_flag:
            for marker_id_logged, data_for_log in all_markers_data_this_frame.items():
                ...
                marker_log_row.extend([f"{v:.6f}" for v in data_for_log.get('cam_quat', [np.nan]*4)]) # Add cam quat
                ...
                if use_reference_marker_flag:
                    if 'rel_pos' in data_for_log and 'rel_rot' in data_for_log:
                        ...
                        marker_log_row.extend([f"{v:.6f}" for v in data_for_log.get('rel_quat', [np.nan]*4)]) # Add rel quat
                    else:
                        marker_log_row.extend(["NaN"] * (6 + 4)) # Euler (3+3) + Quat (4)
        ...
        for buoy_id, estimates_for_this_buoy in detected_markers_for_buoy_fusion.items():
            ...
            rvec_cog_cam = rvec_cam_BRF_final 
            tvec_cog_cam = (final_R_cam_BRF_matrix @ buoy_cog_offset_in_brf_coords) + final_tvec_cam_BRF

            current_log_rvec_cog, current_log_tvec_cog = rvec_cog_cam, tvec_cog_cam
            ...
            if use_reference_marker_flag and ref_marker_detected_this_frame and ref_rvec is not None and ref_tvec is not None:
                try:
                    ...
                    current_log_rvec_cog, current_log_tvec_cog = rvec_cog_relative, tvec_cog_relative
                    is_cog_pose_relative = True
                except Exception: pass 
            
            euler_angles_cog_for_log, quat_cog_for_log = rotation_vector_to_euler(current_log_rvec_cog)
            euler_angles_cog_cam, quat_cog_cam = rotation_vector_to_euler(rvec_cog_cam)
            ...
            if use_reference_marker_flag: 
                ...
                if is_cog_pose_relative: 
                    ...
                    cog_log_entry_data.extend([f"{val[0]:.3f}" for val in euler_angles_cog_for_log])
                    cog_log_entry_data.extend([f"{v:.6f}" for v in quat_cog_for_log]) 
                else: 
                    cog_log_entry_data.extend(["NaN"] * (6 + 4))
                
                cog_log_entry_data.extend([f"{v:.6f}" for v in tvec_cog_cam.flatten()]) 
                cog_log_entry_data.extend([f"{val[0]:.3f}" for val in euler_angles_cog_cam])
            ...
            try:
                ...
                rotations_history[buoy_id].append(euler_angles_cog_for_log) 
                ...
                prefix_text, y_off = ("Rel." if is_cog_pose_relative else "Abs."), -40
                ...
                cv2.putText(annotated_frame, f"{prefix_text}R:({euler_angles_cog_for_log[0,0]:.1f},{euler_angles_cog_for_log[1,0]:.1f},{euler_angles_cog_for_log[2,0]:.1f})", (center_px_cog_on_image[0]+10, center_px_cog_on_image[1]+y_off), cv2.FONT_HERSHEY_SIMPLEX, 0.4, BUOY_COG_INFO_COLOR, 1)
            except Exception: pass 
...
def track_buoy_with_aruco_6dof(parent_tk_window=None):
    ...
    try:
        while True:
            ...
            now = datetime.now(); date_s, time_s = now.strftime("%Y-%m-%d"), now.strftime("%H:%M:%S.%f")[:-3]
...
def process_video_offline(input_video_path, use_ref_marker_override,
    ...
    fr_count, start_t, ref_det_offline = 0, time.time(), False
    ...
    try:
        while True:
            ...
            frame_copy_for_processing = frame.copy()
            vid_ms = cap.get(cv2.CAP_PROP_POS_MSEC)
            vid_ms = vid_ms if vid_ms > 0 or fr_count == 1 else (fr_count - 1) * (1000.0 / fps_orig) 
            
            time_offset_from_video_start = timedelta(milliseconds=vid_ms)
            ...
            if total_fr > 0 and (fr_count % 50 == 0 or fr_count == total_fr):
                el_t = time.time()-start_t; fps_p = fr_count/el_t if el_t > 0 else 0; prog_p = (fr_count/total_fr)*100
...
def calibrate_buoy_marker_geometry(buoy_id_to_calibrate, parent_tk_window=None, num_frames_to_capture=30):
    ...
    for marker_id_Xi_process, data_collected in collected_relative_poses.items():
        ...
        # For rvecs, convert to quaternions, average quaternions, then convert back to rvec
        quats_X0_Xi = []
        for r_val_X0_Xi in data_collected['rvecs']:
            if r_val_X0_Xi is not None:
                ...
                except (cv2.error, ValueError): pass # Skip if rvec is invalid

        if quats_X0_Xi:
            # Use simple weights (e.g., 1 for all) for this calibration step's averaging
            ...
            avg_rvec_X0_Xi = np.array([0.,0.,0.], dtype=np.float32) # Fallback if no valid rvecs
...
class SettingsDialog(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Application Settings"); self.geometry("450x450") 
        self.transient(master); self.grab_set()
