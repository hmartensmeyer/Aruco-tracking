...
## ADDED: Configuration for the reference marker's low-pass filter.
ENABLE_REFERENCE_FILTER = True      # Enable smoothing for the reference marker's pose.
REF_FILTER_ALPHA_T = 0.3            # Smoothing factor for translation (0-1). Lower is smoother.
REF_FILTER_ALPHA_R = 0.3            # Smoothing factor for rotation (0-1). Lower is smoother.
...
BUOY_COG_OFFSET_METERS = 0.37768  #COG : 0.23168 Mooring point : 0.37768
...
## ADDED: A class to implement a simple low-pass filter for poses.
class PoseFilter:
    """
    A simple low-pass filter for 6DOF poses using an Exponential Moving Average.
    This helps to smooth out jitter in pose estimations, especially for static
    or slow-moving objects like a reference marker.
    """
    def __init__(self, alpha_translation=0.5, alpha_rotation=0.5):
        """
        Initializes the filter.
        :param alpha_translation: Smoothing factor for translation (0 to 1).
                                  A lower value means more smoothing (less responsive).
        :param alpha_rotation: Smoothing factor for rotation (0 to 1).
                                 A lower value means more smoothing.
        """
    ...
    def update(self, new_tvec, new_rvec):
        ...
        :param new_tvec: The new translation vector (3x1 or 1x3).
        :param new_rvec: The new rotation vector (3x1 or 1x3).
        :return: A tuple of (filtered_tvec (3x1), filtered_rvec (3x1)).
        """
    ...
    def reset(self):
        """Resets the filter's state. Call this if the object is lost."""
        self.is_initialized = False
        self.filtered_tvec = None
        self.filtered_quat = None
...
def load_settings():
    ...
    try:
        ...
        global REFERENCE_MARKER_ID, USE_REFERENCE_MARKER, ENABLE_REFERENCE_FILTER, REF_FILTER_ALPHA_T, REF_FILTER_ALPHA_R ## MODIFIED
        ...
        ## ADDED: Load filter settings from file
...
def _process_frame_common(
    ...
    plotter=None,
    ## ADDED: Pass the filter object into the core processing function
    ...
    if ids is not None and len(ids) > 0:
        ...
        ## MODIFIED: Entire block for handling the reference marker pose to include filtering.
        if use_reference_marker_flag:
            try:
                ref_idx = np.where(ids.flatten() == reference_marker_id_val)[0][0]
                ref_marker_detected_this_frame = True
                # Get the raw, unfiltered pose from this frame's measurement
                raw_ref_rvec, raw_ref_tvec = rvecs_all[ref_idx].reshape(3,1), tvecs_all[ref_idx].reshape(3,1)

                if ref_pose_filter:
                    # Apply the low-pass filter to smooth the pose
                    ref_tvec, ref_rvec = ref_pose_filter.update(raw_ref_tvec, raw_ref_rvec)
                else:
                    # Use raw, unfiltered values if filter is disabled
                    ref_rvec, ref_tvec = raw_ref_rvec, raw_ref_tvec
                
                # Draw the (potentially filtered) reference marker axes for visualization
                cv2.drawFrameAxes(annotated_frame, camera_matrix, dist_coeffs, ref_rvec, ref_tvec, marker_size_m, AXES_THICKNESS)
            except IndexError:
                # This block executes if the reference marker ID was not found in the 'ids' list
                if ref_pose_filter:
                    ref_pose_filter.reset() # Reset filter state if marker is lost to prevent using stale data
                pass # ref_marker_detected_this_frame remains False, and ref_rvec/tvec remain None
        ...
        for i, marker_id in enumerate(ids.flatten()):
            ...
            if use_reference_marker_flag and ref_marker_detected_this_frame:
                try:
                    # This now uses the filtered ref_rvec and ref_tvec
...
def track_buoy_with_aruco_6dof(parent_tk_window=None):
    ...
    ## ADDED: Initialize the reference marker pose filter if it's enabled in the settings.
    ...
    try:
        while True:
            ...
            ## MODIFIED: Pass the filter object to the processing function.
...
# ... (calibrate_camera and other utility functions remain unchanged)
...
class SettingsDialog(tk.Toplevel):
    def __init__(self, master):
        ...
        ## MODIFIED: Add variables for the new filter settings
        ...
        self.record_video_var, self.reverse_display_var = tk.BooleanVar(value=RECORD_VIDEO), tk.BooleanVar(value=REVERSE_CAMERA_DISPLAY)
        ...
        ## MODIFIED: Restructured the "Tracking & Logging" section to include filter controls
        log_frame = ttk.LabelFrame(main_frame, text="Tracking & Logging", padding="10"); log_frame.pack(fill=tk.X, pady=5)
        c3 = ttk.Checkbutton(log_frame, variable=self.use_ref_marker_var, text="Enable Reference Marker by default", command=self._toggle_ref_widgets); c3.pack(anchor='w', pady=2); ToolTip(c3, "Report poses relative to a static world-frame marker.")
        ref_options_frame = ttk.Frame(log_frame); ref_options_frame.pack(fill=tk.X, padx=20)
        ttk.Label(ref_options_frame, text="Reference Marker ID:").grid(row=0, column=0, sticky='w', padx=5, pady=2)
        self.ref_id_entry = ttk.Entry(ref_options_frame, textvariable=self.ref_marker_id_var, width=7); self.ref_id_entry.grid(row=0, column=1, sticky='w', padx=5, pady=2); ToolTip(self.ref_id_entry, "The ID of the static reference marker.")
        self.ref_filter_check = ttk.Checkbutton(ref_options_frame, variable=self.enable_ref_filter_var, text="Enable Low-Pass Filter", command=self._toggle_ref_widgets); self.ref_filter_check.grid(row=1, column=0, columnspan=2, sticky='w', padx=5, pady=2); ToolTip(self.ref_filter_check, "Smooths the reference marker's pose to reduce jitter.\nAffects all relative calculations.")
        alpha_frame = ttk.Frame(ref_options_frame); alpha_frame.grid(row=2, column=0, columnspan=2, sticky='w', padx=20)
        ttk.Label(alpha_frame, text="T. Alpha:").pack(side=tk.LEFT); self.ref_alpha_t_entry = ttk.Entry(alpha_frame, textvariable=self.ref_filter_alpha_t_var, width=6); self.ref_alpha_t_entry.pack(side=tk.LEFT, padx=(0,10)); ToolTip(self.ref_alpha_t_entry, "Translation smoothing (0-1). Lower is smoother.")
        ttk.Label(alpha_frame, text="R. Alpha:").pack(side=tk.LEFT); self.ref_alpha_r_entry = ttk.Entry(alpha_frame, textvariable=self.ref_filter_alpha_r_var, width=6); self.ref_alpha_r_entry.pack(side=tk.LEFT); ToolTip(self.ref_alpha_r_entry, "Rotation smoothing (0-1). Lower is smoother.")
    ...
    ## MODIFIED: Updated this function to control all reference-related widgets
    ...
    ## MODIFIED: Updated apply function to handle new settings
    def apply_settings(self):
        global USE_REFERENCE_MARKER,REFERENCE_MARKER_ID,RECORD_VIDEO,REVERSE_CAMERA_DISPLAY,CAMERA_INDEX,MARKER_SIZE_METERS,CAMERA_RESOLUTION,LOG_ALL_MARKER_DATA,SHOW_REALTIME_PLOTS,ENABLE_LED_DETECTION,ENABLE_REFERENCE_FILTER,REF_FILTER_ALPHA_T,REF_FILTER_ALPHA_R
        ...
        try:
            ...
            n_alpha_t, n_alpha_r = self.ref_filter_alpha_t_var.get(), self.ref_filter_alpha_r_var.get()
            ...
            if not (0.0 <= n_alpha_t <= 1.0): raise ValueError("Translation Alpha must be between 0.0 and 1.0.")
            if not (0.0 <= n_alpha_r <= 1.0): raise ValueError("Rotation Alpha must be between 0.0 and 1.0.")
        ...
        ENABLE_REFERENCE_FILTER, REF_FILTER_ALPHA_T, REF_FILTER_ALPHA_R = self.enable_ref_filter_var.get(), n_alpha_t, n_alpha_r
        ...
        print(f"Settings updated: Ref={USE_REFERENCE_MARKER},ID={REFERENCE_MARKER_ID},Filter={ENABLE_REFERENCE_FILTER},Rec={RECORD_VIDEO},Rev={REVERSE_CAMERA_DISPLAY},LogAll={LOG_ALL_MARKER_DATA},CamIdx={CAMERA_INDEX},MarkerSize={MARKER_SIZE_METERS},CamRes={CAMERA_RESOLUTION},LED_Sync={ENABLE_LED_DETECTION}")
        save_settings()
