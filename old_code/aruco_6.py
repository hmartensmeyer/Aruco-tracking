...
from collections import deque # For efficient plot data buffering and FPS calculation
...
PLOT_WIDTH_PX = 1500              # Width of the plot panel in pixels. Increased for 4K.
INFO_PANEL_WIDTH_PX = 450        # Width of the info panel. Increased for 4K.

# Font and line sizes for video overlay
OVERLAY_FONT_SCALE = 1.2         # General font scale for cv2.putText. Increased for 4K.
OVERLAY_THICKNESS = 2            # General thickness for cv2.putText and lines.
AXES_THICKNESS = 6               # Thickness for cv2.drawFrameAxes. Increased for 4K.
TRAJECTORY_THICKNESS = 4         # Thickness for buoy trajectory lines. Increased for 4K.
...
# UNWRAP_PLOT_ANGLES = True      # REMOVED - Unwrapping is no longer needed with the stable angle calculation method.
...
OFFLINE_PREVIEW_RESIZE_FACTOR = 1 # Smaller preview for 4K
...
KF_PROCESS_NOISE_VEL_POS = 0.1
KF_PROCESS_NOISE_VEL_ORI = 0.2
KF_MEASUREMENT_NOISE_POS = 0.008
KF_MEASUREMENT_NOISE_ORI = 0.014

#################################
# VISUALIZATION HELPER CLASS
#################################
class InfoPanelDrawer:
    """Handles drawing the consolidated information panel on the frame. Scaled for 4K."""
    def __init__(self, panel_width, font=cv2.FONT_HERSHEY_SIMPLEX, font_scale=OVERLAY_FONT_SCALE, thickness=OVERLAY_THICKNESS):
        ...
        self.y_pos = 0
        self.x_margin = 20
        self.y_step = int(35 * self.font_scale)      # Increased spacing for 4K
        self.y_header_step = int(45 * self.font_scale) # Increased spacing for 4K
        self.frame = None
    ...
    def _draw_text(self, text, color, is_header=False, indent=0):
        if is_header:
            self.y_pos += int(self.y_header_step * 0.6) # Add space before header
            # Make header text slightly larger and bolder
    ...
    def draw(self, frame, proc_fps, cam_res, cam_fps, rec_status, kf_status, ref_status, buoy_info, is_live=True):
        self.frame = frame
        panel = self.frame[:, :self.panel_width]
        cv2.addWeighted(panel, 0.5, np.zeros_like(panel), 0.5, 0, panel) # Semi-transparent overlay
        self._reset_y()
        ...
        rec_color = self.colors['green'] if rec_on else self.colors['red']
        self._draw_text(f"Recording: {'ON' if rec_on else 'OFF'}", rec_color)
        if rec_on and rec_file:
            self._draw_text(os.path.basename(rec_file), self.colors['orange'], indent=20)

        kf_on = kf_status
        kf_color = self.colors['green'] if kf_on else self.colors['red']
        self._draw_text(f"Kalman Filter: {'ENABLED' if kf_on else 'DISABLED'}", kf_color)

        ref_on, ref_found, ref_id = ref_status
        ref_color = self.colors['green'] if ref_on else self.colors['red']
        self._draw_text(f"Reference: {'ENABLED' if ref_on else 'DISABLED'}", ref_color)
        if ref_on:
            ref_found_color = self.colors['green'] if ref_found else self.colors['red']
            self._draw_text(f"  > ID {ref_id}: {'Found' if ref_found else 'Lost'}", ref_found_color, indent=20)
        ...
        if not buoy_info:
            ...
            for buoy_id, info in sorted_buoys:
                ...
                # Draw the quality circle - larger for 4K
                ...
                # Draw the text with larger indent
                buoy_text = f"Buoy {buoy_id}: {num_markers} marker(s)"
                self._draw_text(buoy_text, self.colors['white'], indent=40)
...
class RealTimePlotter:
    """
    Handles the creation and updating of real-time data plots.
    MODIFIED: Removed angle unwrapping to use stable angle inputs directly.
    """
    def __init__(self, history_len, plot_width_px, video_height_px, buoy_ids_to_plot, is_relative):
        ...
        self.buoy_data = {}
        self.buoy_lines = {}
        # NOTE: self.last_angles is no longer needed
        ...
        # UPDATED Title
        rot_title = "Buoy CoG Orientation"
        self.ax_rot.set_title(rot_title, fontsize=PLOT_TITLE_FONTSIZE)
        ...
        for i, buoy_id in enumerate(buoy_ids_to_plot):
            ...
            # UPDATED Labels for new stable angles
            rot_lines = {
    ...
    def _canvas_to_numpy(self):
        """Renders the matplotlib canvas to a NumPy array."""
    ...
    def update_and_get_plot_image(self, new_data_dict, timestamp):
        ...
        for buoy_id, data in new_data_dict.items():
            ...
            # --- Rotation Data (SIMPLIFIED - no unwrapping) ---
            # data['rot'] will now be the stable angles
            stable_rot = data['rot']
...
class KalmanFilterPose6DOF_Quat:
    ...
    def predict(self, dt):
        ...
        # 1. Predict error state covariance
...
def get_video_writer(frame_size, full_video_path, fps=None):
    ...
    try:
        ...
        if writer.isOpened():
            print(f"Video writer created for: {full_video_path} with codec {codec_to_try} and size {frame_size}.")
            return writer
        else:
...
def rotation_vector_to_euler(rvec):
    """Converts a rotation vector to XYZ Euler angles and a quaternion. Used for logging and on-screen text."""
    if rvec is None:
...
def get_stable_orientation_angles_from_matrix(rot_matrix):
    """
    Calculates a set of more stable orientation angles from a rotation matrix.
    This avoids the main gimbal lock issues for plotting.

    Args:
        rot_matrix (np.array): A 3x3 rotation matrix that transforms points
                               from the body frame to the world/camera frame.

    Returns:
        tuple: (yaw, pitch, inclination) in degrees.
          - yaw: Rotation around the world Z-axis. Stable.
          - pitch: Angle of the body's forward axis with the world horizontal plane.
          - inclination: Total tilt angle of the buoy's 'up' axis from the vertical.
    """
    # Define the body's principal axes in its own coordinate system
    body_x_axis = np.array([1., 0., 0.]) # Forward vector of the buoy
    body_z_axis = np.array([0., 0., 1.]) # Up vector of the buoy

    # Use the rotation matrix to find where these axes point in the world/camera frame
    body_x_in_world = rot_matrix @ body_x_axis
    body_z_in_world = rot_matrix @ body_z_axis

    # 1. Yaw (Heading): Project the body's forward axis onto the world's XY plane
    # and calculate the angle from the world's X-axis. This is stable.
    yaw = np.rad2deg(np.arctan2(body_x_in_world[1], body_x_in_world[0]))

    # 2. Pitch: Calculate the angle of the body's forward axis with the
    # world's horizontal plane (the XY plane).
    # This is the arcsin of the z-component of the transformed forward vector.
    pitch = np.rad2deg(np.arcsin(-body_x_in_world[2]))

    # 3. Inclination (Total Tilt): A very stable measure of how much the buoy is tilted.
    # It's the angle between the body's "up" vector (body Z) and the world's "up" vector (world Z).
    # The dot product is just the z-component of the transformed z-vector.
    cos_inclination = body_z_in_world[2]
    # Clamp the value to avoid floating-point domain errors with arccos
    inclination = np.rad2deg(np.arccos(np.clip(cos_inclination, -1.0, 1.0)))
...
def _process_frame_common(
    ...
    if ids is not None and len(ids) > 0:
        ...
        if use_reference_marker_flag:
            for i, marker_id_array in enumerate(ids):
                if marker_id_array[0] == reference_marker_id_val:
                    ...
                    # Draw larger, thicker axes for visibility
        ...
        for buoy_id, estimates_for_this_buoy in detected_markers_for_buoy_fusion.items():
            ...
            rvec_cog_cam_raw, _ = cv2.Rodrigues(final_R_cam_BRF_matrix)
            ...
            if use_kalman_filter_flag:
                ...
                quat_cog_cam = filtered_state_cam_pose[3:7]
                try:
                    rvec_cog_cam = R.from_quat(quat_cog_cam).as_rotvec().reshape(3,1)
                except Exception:
                    pass # Keep previous rvec if conversion fails

            # --- START OF ANGLE CALCULATION REFACTOR ---
            # Get the final, filtered rotation matrix of the buoy CoG in the camera frame
            try:
                final_rot_matrix_cog_cam = R.from_quat(quat_cog_cam).as_matrix()
            except Exception as e:
                print(f"Error converting filtered quaternion for buoy {buoy_id}: {e}")
                final_rot_matrix_cog_cam, _ = cv2.Rodrigues(rvec_cog_cam_raw) # Fallback
            ...
            # --- Data for CSV and On-Screen Text (using original unstable Euler angles for consistency) ---
            ...
            # --- END OF ANGLE CALCULATION REFACTOR ---

            # The rest of the function for logging and drawing on screen remains the same.
            cog_log_entry_data = [log_date_str, log_time_str, buoy_id]
            # Get the camera-frame euler angles for logging, regardless of relative mode
            euler_angles_cog_cam_deg_for_log, quat_cog_cam_for_log = rotation_vector_to_euler(rvec_cog_cam)
            ...
            if use_reference_marker_flag:
                ...
                cog_log_entry_data.extend([f"{val[0]:.3f}" for val in euler_angles_cog_cam_deg_for_log])
                cog_log_entry_data.extend([f"{v:.6f}" for v in quat_cog_cam_for_log])
            else:
                cog_log_entry_data.extend([f"{v:.6f}" for v in tvec_cog_cam.flatten()])
                cog_log_entry_data.extend([f"{val[0]:.3f}" for val in euler_angles_cog_cam_deg_for_log])
                cog_log_entry_data.extend([f"{v:.6f}" for v in quat_cog_cam_for_log])
        ...
        for i in range(1, len(pos_list_hist)):
            if pos_list_hist[i-1] is not None and pos_list_hist[i] is not None:
                try:
                    ...
                    cv2.line(annotated_frame, pt1, pt2, TRAJECTORY_COLOR, TRAJECTORY_THICKNESS)
                except Exception: pass
...
def track_buoy_with_aruco_6dof(parent_tk_window=None):
    ...
    try:
        while True:
            ...
            # Add timestamp to the video part of the frame (top right) - scaled for 4K
...
def process_video_offline(input_video_path, use_ref_marker_override,
    ...
    try:
        while True:
            ...
            elif current_frame_time_msec < 0 :
            ...
            # Add timestamp and progress to top right - scaled for 4K
            ...
            if SHOW_OFFLINE_PROCESSING_PREVIEW:
                if w*OFFLINE_PREVIEW_RESIZE_FACTOR > 0 and h*OFFLINE_PREVIEW_RESIZE_FACTOR > 0 :
                    try:
                        disp_res = cv2.resize(final_output_frame, (int(output_frame_size[0]*OFFLINE_PREVIEW_RESIZE_FACTOR), int(output_frame_size[1]*OFFLINE_PREVIEW_RESIZE_FACTOR)))
...
class SettingsDialog(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Application Settings"); self.geometry("500x700")
        self.transient(master); self.grab_set()
        ...
        kf_frame = ttk.LabelFrame(main_frame, text="Kalman Filter (Buoy CoG)", padding="5")
        kf_frame.grid(row=r, column=0, columnspan=3, sticky="ew", pady=5); r+=1
        ttk.Checkbutton(kf_frame, variable=self.use_kalman_filter_var, text="Enable Kalman Filter").grid(row=0, column=0, columnspan=2, sticky=tk.W, pady=2)
        ttk.Label(kf_frame, text="Proc. Noise Vel Pos (m/s):").grid(row=1, column=0, sticky=tk.W, pady=2)
        ttk.Entry(kf_frame, textvariable=self.kf_proc_noise_vel_pos_var, width=10).grid(row=1, column=1, sticky=tk.W, pady=2)
        ttk.Label(kf_frame, text="Proc. Noise Vel Ori (rad/s):").grid(row=2, column=0, sticky=tk.W, pady=2)
        ttk.Entry(kf_frame, textvariable=self.kf_proc_noise_vel_ori_var, width=10).grid(row=2, column=1, sticky=tk.W, pady=2)
        ttk.Label(kf_frame, text="Meas. Noise Pos (m):").grid(row=3, column=0, sticky=tk.W, pady=2)
        ttk.Entry(kf_frame, textvariable=self.kf_meas_noise_pos_var, width=10).grid(row=3, column=1, sticky=tk.W, pady=2)
        ttk.Label(kf_frame, text="Meas. Noise Ori (rad):").grid(row=4, column=0, sticky=tk.W, pady=2)
        ttk.Entry(kf_frame, textvariable=self.kf_meas_noise_ori_var, width=10).grid(row=4, column=1, sticky=tk.W, pady=2)
    ...
    def apply_settings(self):
        ...
        global KF_MEASUREMENT_NOISE_POS, KF_MEASUREMENT_NOISE_ORI, SHOW_REALTIME_PLOTS
...
if __name__ == "__main__":
    ...
    print(f"Using SciPy version: {R.random().as_matrix().shape}")
